...
// Consulta la base de datos para obtener los pedidos recibidos con la misma fecha que hoy
try {
    $estadoRecibido = "Received";
    $fechaHoy = date("Y-m-d H:i:s"); // Obtiene la fecha y hora actual
    $fechaInicio = date("Y-m-d 00:00:00", strtotime($fechaHoy));
    $fechaFin = date("Y-m-d 23:59:59", strtotime($fechaHoy));

    $query = "SELECT * FROM pedidos WHERE estado = :estado AND fecha_pedido >= :fechaInicio AND fecha_pedido <= :fechaFin";
        ...
        <div class="pedidos-container">
            <?php foreach ($pedidosRecibidos as $pedido) : ?>
                <div class="pedido-card animate__animated  animate__fadeIn animate__faster	">
                    ...
                    <form action="actualizar_estado1.php" method="POST">
                        <input type="hidden" name="pedido_id" value="<?php echo $pedido['id']; ?>">
                        <button type="submit" name="enviar_en_proceso">Send to In Process
                        </button>
                    </form>
                    <form action="actualizar_cancelar.php" method="POST">
                        <input type="hidden" name="pedido_id" value="<?php echo $pedido['id']; ?>">
                        <button type="submit" name="enviar_cancelados">Cancel Order
                        </button>
                    </form>
                </div>
            <?php endforeach; ?>
