...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["enviar_en_proceso"]) && isset($_POST["pedido_id"])) {
    ...
    try {
        ...
        $estadoEnProceso = "In Process";
        $query = "UPDATE pedidos SET estado = :estado WHERE id = :pedidoId";
        $stmt = $pdo->prepare($query);
        $stmt->bindParam(':estado', $estadoEnProceso, PDO::PARAM_STR);
        $stmt->bindParam(':pedidoId', $pedidoId, PDO::PARAM_INT);

        if ($stmt->execute()) {
            header("Location: en_proceso.php"); // Redirige de nuevo a la página de pedidos recibidos
            exit();
        } else {
